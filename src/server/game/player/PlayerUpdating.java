...
public final class PlayerUpdating {
	...
	public static void update(Player player) {
		...
		player.getEncoder().send(out.getBuffer());
	}
	...
	public static void appendAppearance(Player player, StreamBuffer.OutBuffer out) {
		...
		block.writeByte(0); // prayer icon
		block.writeByte(0); // Skull icon

		// Hat.
		Item helm = player.getEquipment()[Misc.EQUIPMENT_SLOT_HEAD];
		if (helm != null) {
		...
		// Cape.
		Item item = player.getEquipment()[Misc.EQUIPMENT_SLOT_CAPE];
		if (item != null) {
		...
		// Amulet.
		item = player.getEquipment()[Misc.EQUIPMENT_SLOT_AMULET];
		if (item != null) {
			block.writeShort(0x200 + item.getId());
		} else {
			block.writeByte(0);
		...
		// Weapon.
		item = player.getEquipment()[Misc.EQUIPMENT_SLOT_WEAPON];
		if (item != null) {
			block.writeShort(0x200 + item.getId());
		} else {
			block.writeByte(0);
		...
		// Chest.
		Item chest = player.getEquipment()[Misc.EQUIPMENT_SLOT_CHEST];
		if (chest != null) {
		...
		// Shield.
		item = player.getEquipment()[Misc.EQUIPMENT_SLOT_CHEST];
		if (item != null) {
			block.writeShort(0x200 + item.getId());
		} else {
			block.writeByte(0);
		...
		// Legs.
		item = player.getEquipment()[Misc.EQUIPMENT_SLOT_LEGS];
		if (item != null) {
			block.writeShort(0x200 + item.getId());
		} else {
		...
		// Hands.
		item = player.getEquipment()[Misc.EQUIPMENT_SLOT_HANDS];
		if (item != null) {
			block.writeShort(0x200 + item.getId());
		} else {
		...
		// Feet.
		item = player.getEquipment()[Misc.EQUIPMENT_SLOT_FEET];
		if (item != null) {
			block.writeShort(0x200 + item.getId());
		} else {
		...
		// Beard.
		item = player.getEquipment()[Misc.EQUIPMENT_SLOT_HEAD];
		if (player.getGender() == Misc.GENDER_MALE && (helm == null || (helm != null && !helm.getDefinition().isFullMask()))) {
